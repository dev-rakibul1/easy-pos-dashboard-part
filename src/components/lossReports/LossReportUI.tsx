...
const LossReportUI = ({ lossFilter, loading, dateRange }: IProps) => {
  const { token } = theme.useToken()
  const data = lossFilter ? lossFilter?.sells : []
  ...
  const calculateAllLoss = (data: any[]) => {
    const lossArray = data?.map(item => {
      ...
      const loss = purchaseRate - sellWithDiscount
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => {
        ...
        return parseFloat(loss.toString()).toFixed(2)
      },
    },
  ]
                ...
                return (
                  <Table.Summary.Row>
                    ...
                    <Table.Summary.Cell index={8}>
                      <strong>{calculateAllLoss(data)?.toFixed(2)}</strong>
                    </Table.Summary.Cell>
                  </Table.Summary.Row>
                )
              }}
